...
export function parsePortalBuildMetadata(text) {
...
export async function readDeployedPortalBuild(
  targetUrl,
  {
  ...
  if (
    url.protocol !== 'https:' ||
    url.hostname !== 'asv.acecore.net' ||
    url.pathname !== '/.well-known/aceserver-portal-build.json'
  ) {
    throw new Error('Portal build marker URL is not allowed.')
  }
  ...
  return parsePortalBuildMetadata(await response.text())
}

export async function assertDeployedPortalBuild(
  targetUrl,
  expectedCommit,
  ...
  if (!CORPUS_VERSION_PATTERN.test(expectedCorpusVersion)) {
    throw new Error(
      'Expected corpus version must contain 20 hexadecimal characters.',
    )
  }
  ...
  const deployed = await readDeployedPortalBuild(targetUrl, {
    fetchImpl,
    fetchTimeoutMs,
  })
  if (
    ...
    throw new Error(
      'Production changed or its portal corpus differs from this workflow build.',
    )
  }

  logger.log(
    JSON.stringify({ event: 'portal_pages_build_confirmed', ...expected }),
  )
  return deployed
}

export async function waitForPortalDeployment(
  targetUrl,
  expectedCommit,
  {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (deployed.commit === normalizedExpectedCommit) {
        logger.log(
          JSON.stringify({
            event: 'portal_pages_deployment_ready',
...
if (isDirectExecution()) {
  ...
  if (!targetUrl || !command) {
    throw new Error(
      'Usage: node scripts/wait-for-portal-deployment.mjs <marker-url> <commit-sha|--print-current|--assert-current> [commit-sha corpus-file]',
    )
  }

  if (command === '--print-current') {
    console.log((await readDeployedPortalBuild(targetUrl)).commit)
    ...
    await assertDeployedPortalBuild(targetUrl, expectedCommit, corpus?.version)
  } else {
    await waitForPortalDeployment(targetUrl, command)
  }
}
