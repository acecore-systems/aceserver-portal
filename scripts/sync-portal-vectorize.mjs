...
export async function syncPortalVectorize({
  ...
  if (dryRun) {
    ...
    logger.log(JSON.stringify({ event: 'portal_vectorize_dry_run', ...result }))
    return result
  }
...
export function validatePortalCorpus(corpus) {
  if (
    corpus?.schemaVersion !== PORTAL_CORPUS_SCHEMA_VERSION ||
    corpus?.embedding?.model !== PORTAL_EMBEDDING_MODEL ||
    corpus?.embedding?.dimensions !== PORTAL_EMBEDDING_DIMENSIONS ||
    corpus?.embedding?.metric !== PORTAL_DISTANCE_METRIC
  ) {
    throw new Error(
      `Corpus must use schema v${PORTAL_CORPUS_SCHEMA_VERSION}, ${PORTAL_EMBEDDING_MODEL}, ${PORTAL_EMBEDDING_DIMENSIONS} dimensions, and ${PORTAL_DISTANCE_METRIC}.`,
    )
  }
  if (
    ...
    throw new Error('Corpus version must be a 20-character hex digest.')
  }
  if (
    !Number.isInteger(corpus.sourceCount) ||
    corpus.sourceCount < PORTAL_MIN_SOURCE_COUNT
  ) {
    throw new Error(
      `Corpus must contain at least ${PORTAL_MIN_SOURCE_COUNT} source documents.`,
    )
  }
  if (
    ...
    corpus.vectorCount < PORTAL_MIN_VECTOR_COUNT ||
    corpus.vectorCount > PORTAL_VECTOR_LIMIT
  ) {
    throw new Error(
      `Corpus vector count must be between ${PORTAL_MIN_VECTOR_COUNT} and ${PORTAL_VECTOR_LIMIT}.`,
    )
  }
  if (corpus?.localeCounts?.[PORTAL_SEARCH_NAMESPACE] !== corpus.vectorCount) {
    throw new Error('Corpus Japanese locale count must match vectorCount.')
  }
  ...
  for (const chunk of corpus.chunks) {
    if (
      ...
      chunk?.namespace !== PORTAL_SEARCH_NAMESPACE ||
      typeof chunk?.text !== 'string' ||
      !chunk.text.trim() ||
      chunk.text.length > 1200 ||
      !isValidPortalMetadata(chunk.metadata)
    ) {
  ...
  if (sourceUrls.size !== corpus.sourceCount) {
    throw new Error('Corpus sourceCount must match unique chunk URLs.')
  }
}
...
function isValidPortalMetadata(metadata) {
  ...
  return Boolean(
    metadata.locale === PORTAL_SEARCH_NAMESPACE &&
    isBoundedString(metadata.title, MAX_METADATA_TITLE_LENGTH) &&
    isBoundedString(metadata.section, MAX_METADATA_SECTION_LENGTH) &&
    isBoundedString(metadata.excerpt, MAX_METADATA_EXCERPT_LENGTH) &&
    isBoundedString(metadata.contentType, MAX_METADATA_CONTENT_TYPE_LENGTH) &&
    url.length <= MAX_METADATA_URL_LENGTH &&
    /^\/(?!\/)/u.test(url) &&
    url.endsWith('/') &&
    !url.includes('\\') &&
    !url.includes('?') &&
    !url.includes('#') &&
    !/%(?:2f|5c)/iu.test(url) &&
    !/^\/(?:admin|api)(?:\/|$)/u.test(url) &&
    !['/404/', '/404.html/', '/vector-corpus.json/'].includes(url),
  )
}
...
function validateIndexConfiguration(index, indexName) {
  ...
  if (
    config?.dimensions !== PORTAL_EMBEDDING_DIMENSIONS ||
    config?.metric !== PORTAL_DISTANCE_METRIC
  ) {
    throw new Error(
      `Vectorize index ${indexName} must use ${PORTAL_EMBEDDING_DIMENSIONS} dimensions and ${PORTAL_DISTANCE_METRIC}.`,
    )
  }
}
    ...
    async request(path, init = {}) {
      ...
      for (let attempt = 0; attempt <= MAX_REQUEST_RETRIES; attempt += 1) {
        ...
        try {
          ...
          if (
            attempt >= MAX_REQUEST_RETRIES ||
            !isRetryableNetworkError(error, controller.signal.aborted)
          ) {
            throw error
          }
          await sleepImpl(
...
async function listVectorIds(
  ...
  for (let restart = 0; restart <= MAX_LIST_CURSOR_RESTARTS; restart += 1) {
    try {
      ...
      logger.log(
        JSON.stringify({
          event: 'portal_vectorize_list_cursor_restart',
          indexName,
          restart: restart + 1,
        }),
      )
...
function getMutationId(payload) {
  const mutationId = payload?.result?.mutationId ?? payload?.mutationId
  if (typeof mutationId !== 'string' || !mutationId.trim()) {
    throw new Error('Vectorize mutation response did not include mutationId.')
  }
  return mutationId
}
...
if (isDirectExecution()) {
  await syncPortalVectorize(parseArguments(process.argv.slice(2)))
}
