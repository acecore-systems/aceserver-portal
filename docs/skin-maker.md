...
- 修正では`patches`の局所座標を使用する。**修正を拡縮しない。** 選択した部位・面・レイヤー以外への書き込み、面からのはみ出し、重複パッチを拒否する。全検証成功後にコピーへ反映し、パッチ外のRGBAをバイト単位で維持する。モデルには元アトラス画像と、編集可能面の正確なRGBAも渡す。
...
5. 予約後のみ`env.AI.run('@cf/zai-org/glm-5.3-flash', ...)`を1回呼ぶ。出力上限12,000 tokens、`reasoning_effort=low`、`store=false`、サーバー側150秒timeout。クライアント待ちは180秒。自動retry・他モデルへのfallbackはしない。
...
文章のみの試行にはCloudflare開発接続の内部エラー、140.7秒で返った色記号不整合、160秒の待機上限があった。自然言語修正はJSON Modeなしの試行で31.2秒・96出力tokensのJSONコード枠付き応答が返ったが、その後の検証呼び出しでは内部エラーや接続エラーが発生した。単一コード枠を受理するパーサーへ修正済みだが、最終コードでの修正成功は未確認。**文章生成・自然言語修正の実モデル成功と安定性は未確認。最終面辞書形式の新規生成を含め、公開前の実モデル再評価が必要。** 自動テストでの成功を実モデル成功と混同しない。
