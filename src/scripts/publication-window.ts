...
declare global {
  interface Window {
    __asvPublicationWindowRuntimeInitialized?: boolean
  }
}
...
function getEarlierTime(current: number | null, candidate: number): number {
...
export function initPublicationWindowRuntime(): void {
  if (window.__asvPublicationWindowRuntimeInitialized) {
    refreshPublicationWindows()
    return
  }

  window.__asvPublicationWindowRuntimeInitialized = true
  initPublicationWindows()
...
function initPublicationWindows(): void {
