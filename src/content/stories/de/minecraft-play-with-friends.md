---
title: 'Minecraft mit Freunden spielen: Java, Bedrock, Switch und Handy richtig wählen'
description: Kläre den Unterschied zwischen Java und Bedrock, vergleiche eigene Welten, Realms und öffentliche Server und wähle für jedes Gerät den passenden Weg zum Spielen mit Freunden.
translationOf: minecraft-play-with-friends
sourceHash: sha256:ad96ebc65f402126753dcb26d69cef1efa89c68b2510ddd1d93d2d77822cb646
date: 2026-08-08T10:00:00+09:00
tags:
  - Minecraft
  - Mehrspieler
  - Erste Schritte
author: Gui
image: /uploads/stories/minecraft-play-with-friends-hero.webp
imageAlt: Vier Freunde beraten sich an einer Karte auf einem Platz im Blockstil bei Sonnenuntergang, neben ihnen stehen ein PC, ein Handheld und ein Smartphone
---

Bevor ihr entscheidet, wo ihr Minecraft mit Freunden spielt, prüft zuerst, ob alle dieselbe Edition nutzen können: **Java Edition oder Bedrock Edition**. Dieser Leitfaden vergleicht eigene Welten, Realms und öffentliche Server und hilft PC-, Switch- und Mobilspielern bei der Auswahl.

## Zuerst prüfen: Java und Bedrock spielen nicht standardmäßig zusammen

### Java Edition spielt mit Java Edition

Die Java Edition ist vor allem die PC-Edition für Windows, macOS und Linux. Java-Welten, Realms und Server nehmen Java-Spieler auf. Auch wenn ein Freund Minecraft auf einem PC besitzt, kann ein Bedrock-Client einem Java-Ziel nicht standardmäßig beitreten.

### Bedrock Edition spielt mit Bedrock Edition

Die Bedrock Edition wird auf Windows-PCs, Switch, Telefonen und Konsolen verwendet. Wenn die Gruppe einen Switch- oder Handyspieler enthält, bestätigt zuerst, dass alle Bedrock nutzen. Die offiziellen [Mehrspieler-Anforderungen](https://help.minecraft.net/hc/en-us/articles/35930809028749-Requirements-to-Play-Minecraft-Multiplayer-Games) sagen, dass die installierte Edition zum Ziel passen muss.

## Vier Prüfungen vor dem Beitreten

1. **Nutzt dieselbe Edition.** Entscheidet zuerst, ob die Gruppe Java oder Bedrock spielt. Behandelt Java-zu-Bedrock-Beitritte nicht als Standardfall.
2. **Aktualisiert das Spiel.** Ein Versionsunterschied kann verhindern, dass eine Welt erscheint oder ein Spieler beitritt, selbst innerhalb derselben Edition.
3. **Prüft Konten und Online-Bedingungen.** Switch, Mobilgeräte und andere Plattformen benötigen für Mehrspieler eine Microsoft-Konto-Anmeldung oder -Verknüpfung. Prüft jedes Gerät in der offiziellen [Konto- und Editionstabelle](https://help.minecraft.net/hc/en-us/articles/19615552270221-Accounts-Required-to-Play-Minecraft). Konsolen-Mehrspieler online benötigt außerdem das Online-Abo der Plattform.
4. **Prüft Kinderkonten.** Eine von Eltern verwaltete Mehrspieler-Berechtigung kann Einladungen und Beitritte blockieren. Teilt niemals Passwörter; prüft die Privatsphäre mit einem Elternteil oder Erziehungsberechtigten.

## Zwischen eigener Welt, Realms und öffentlichem Server wählen

| Option              | Gut geeignet für                                                | Zustand des Hosts                                               | Vor dem Spielen prüfen                                                 |
| ------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Eigene Welt         | Eine kleine Gruppe für heute Abend                              | Andere treten bei, solange der Besitzer die Welt geöffnet hält  | Gleiche Edition sowie LAN- oder Einladungsbedingungen                  |
| Realms              | Freunde, die zu unterschiedlichen Zeiten dieselbe Welt betreten | Eingeladene können mit aktivem Abo auch ohne Besitzer beitreten | Java- oder Bedrock-Realm, Einladung und Abo                            |
| Öffentlicher Server | Eine dauerhafte Gemeinschaft mit weiteren Spielern              | Der Betreiber verwaltet die Welt                                | Unterstützte Edition, Beitrittsweg, Regeln, Alter und aktueller Status |

### 1. Eigene Welt: zuerst in kleiner Runde starten

Für Freunde im selben Haus oder am selben Ort ist das Öffnen einer Welt über LAN eine einfache erste Wahl. Bei Bedrock verwendet ihr kompatible Geräte im selben lokalen Netzwerk und aktiviert beim Host Mehrspieler und LAN-Sichtbarkeit. Der offizielle [Bedrock-LAN-Leitfaden](https://help.minecraft.net/hc/en-us/articles/35697355112717) erklärt, wie ihr nach der Verbindung mit demselben Netzwerk eine Welt findet.

Auch das Einladen von Online-Freunden verwendet die geöffnete Welt des Hosts. Das passt zu zwei Personen, die heute mit Bauen anfangen oder erst testen möchten, ob die Gruppe denselben Spielstil mag. Schließt der Host die Welt, können die anderen nicht mehr hinein; plant daher nach seiner Spielzeit.

### 2. Realms: weiterspielen, wenn der Besitzer weg ist

Realms ist das offizielle kostenpflichtige Hosting von Minecraft. Bei aktivem Abo können eingeladene Freunde auch dann eintreten, wenn der Besitzer das Spiel nicht ausführt. Java Realms und Bedrock Realms sind getrennt, also wählt das passende für die Gruppe. Für Java gibt es den offiziellen [Leitfaden zum Erstellen eines Java Realm](https://help.minecraft.net/hc/en-us/articles/4409253767821-How-to-Create-a-Minecraft-Java-Edition-Realm), für Bedrock den offiziellen [Leitfaden für Bedrock-Realm-Einladungslinks](https://help.minecraft.net/hc/en-us/articles/36974952310925-Manage-Invite-Links-for-Minecraft-Bedrock-Edition-Realms).

Das passt zu Freunden mit unterschiedlichen Schul-, Arbeits- oder Vereinszeiten, die eine gemeinsame Basis schrittweise ausbauen möchten. Veröffentlicht einen Einladungslink nicht dort, wo ihn jeder sehen kann; deaktiviert oder entfernt ihn, wenn er nicht mehr gebraucht wird.

### 3. Öffentliche Server: einer Community beitreten

Ein öffentlicher Server ist eine Online-Welt, die von einem Betreiber verwaltet wird. Er kann für Menschen passen, die mit noch unbekannten Spielern bauen, leben oder Veranstaltungen besuchen möchten. Prüft vor dem Beitreten Regeln, unterstützte Edition und Version, Altersbedingungen sowie den Umgang mit Sprachchat oder Discord.

Switch-Spieler sollten besonders die Hinweise des Servers zu unterstützten Geräten und dem Beitrittsweg lesen. Setzt keine inoffizielle Verbindungslösung voraus. Auch wenn ein Server Java und Bedrock bewirbt, hängen die tatsächlichen Anforderungen und unterstützten Versionen von diesem Server ab.

## Start nach Java, Bedrock, Switch oder Mobilgerät

### Java Edition auf dem PC spielen

Wenn alle Java Edition nutzen, wählt eine LAN-Welt, einen Java Realm oder einen Java-Server. Prüft vor der Einladung, dass Freunde Java Edition gestartet haben und die Spielversionen übereinstimmen.

### Bedrock Edition auf dem PC spielen

Windows-PC-Spieler mit Bedrock können mit Bedrock-Spielern auf Switch und Handy in derselben Gruppe sein. Wählt je nach Zeit und Ziel der Gruppe eine kurze Sitzung in eurer Welt, einen dauerhaften Realm oder einen öffentlichen Server.

### Auf Nintendo Switch spielen

Minecraft auf Switch ist Bedrock Edition. Prüft vor Online-Mehrspieler Nintendo Account, Microsoft-Kontoverknüpfung, Nintendo Switch Online und das Spiel-Update. Eine Gruppe mit Switch-Spieler sollte normalerweise Bedrock statt Java nutzen.

### Auf Telefon oder Tablet spielen

Minecraft auf Mobilgeräten ist ebenfalls Bedrock Edition. Prüft unter Android das Google-Konto, auf iPhone und iPad die Apple-ID und meldet euch dann für Mehrspieler am Microsoft-Konto an oder verknüpft es. Eine kurze LAN-Sitzung im heimischen WLAN erleichtert die Fehlersuche, bevor ihr zu Realms oder einem öffentlichen Server wechselt.

## Schnell entscheiden

- **Die Gruppe enthält Switch- oder Mobilspieler:** Alle nutzen Bedrock.
- **Alle haben Java Edition auf dem PC:** Wählt Java-Welt, Realm oder Server.
- **Ihr möchtet ohne Host weiterspielen:** Betrachtet Realms.
- **Ihr möchtet neue Spieler treffen:** Wählt nach dem Lesen der Regeln und der unterstützten Edition einen öffentlichen Server.

Einige Server bieten ihr eigenes zusätzliches System für Java-Bedrock-Crossplay an. Das ist kein Standardversprechen allein aufgrund der Geräte; bestätigt den aktuellen Support in der offiziellen Anleitung des Ziels.

## Mit Freunden auf Aceserver spielen

Wenn ihr einen öffentlichen Server wählt, prüft zuerst den aktuellen Einstieg im [Aceserver-Portal](/de/) und lest dann die aktuellen Beitrittshinweise und Regeln im [offiziellen Discord](https://discord.gg/acsv) und im [offiziellen WIKI](https://asv-wiki.acecore.net). Dieser Artikel kopiert keine veränderlichen Verbindungsdaten; die offiziellen Hinweise sind maßgeblich.

Wenn ihr Freunde einladet, bestätigt zuerst dieselbe Edition, lest die Regeln vor dem Beitritt und teilt niemals Kontoinformationen. Um zuerst öffentliche Server zu vergleichen, lest [wie man einen Minecraft-Server auswählt](/de/stories/minecraft-server-osusume/).
