...
`npm run build` は `dist/vector-corpus.json` まで生成します。`npm run sync:portal-vectorize:dry-run` でsource数、vector数、corpus versionを確認できます。実同期先は現行の `aceserver-portal-search-openai-1536-production` と置換先の `aceserver-portal-search-openai-1536-production-v2` だけに制限し、対象index名と完全一致する `--confirm-production` がない実行、管理外ID、20%を超える削除、10 source未満のcorpusでは停止します。

`.github/workflows/sync-portal-vectorize.yml` はProduction専用です。GitHub連携のPagesで公開されたcommitとcorpus versionを照合してから置換先へ同期し、全vector IDの一致と実query canaryを確認します。移行中の `PORTAL_SEARCH_INDEX` bindingは現行indexのまま保ち、置換先の検証成功後に別PRで切り替えます。通常は`main`へのpush、取りこぼしは6時間ごとのscheduleで再照合し、20%超の削除をworkflowから上書きする経路は設けません。切替後も現行indexはロールバック用に保持します。
