...
`functions/api/alpha-chat.ts` は同一origin検証とD1レート制限を行った後、Private Service Binding `ALPHA_CHAT_SERVICE` を通じて共有Worker `aceserver-alpha-chat` だけを呼びます。共有Workerが人格、口調、質問分類、RAG、引用検証、現在選択中のOpenAI API直結GPT-6 Lunaによる生成、正史作成を一元管理します。将来は検証済みのCloudflare Workers AI文章モデルも明示設定できます。Portal側の会話model切替flagやローカルLLM生成はなく、Service Bindingがない場合や共有Workerの応答が壊れている場合は、選択localeの固定案内を返してfail closedします。
