...
LF正規化SHA-256、変更可能な翻訳ファイルの完全一致、GitHub Actions由来の
`Build and Format`成功をすべて確認した場合だけsquash auto-mergeを予約します。
...
`TRANSLATION_BOT_APP_PRIVATE_KEY`を使います。CIやPR内容のreadは権限を分離した
Actionsの`GITHUB_TOKEN`で行うため、Translation BotへChecks権限は付与しません。
